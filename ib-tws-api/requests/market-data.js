...
function request_mktData(serverVersion, p) {
  ...
  if (p.contract.secType == "BAG" && p.contract.comboLegs.length > 0) {
    let comboLegsCount = p.contract.comboLegs.length;
...
function request_tickByTickData(serverVersion, p) {
  /*
  contract: Contract,
  tickType: str,
  numberOfTicks: int,
...
function request_cancelTickByTickData(serverVersion, requestId) {
  if (serverVersion < ServerVersion.MIN_SERVER_VER_TICK_BY_TICK) {
    throw new Error("It does not support tick-by-tick data requests.");
  }
