    ...
    if (this.serverVersion >= ServerVersion.MIN_SERVER_VER_PEGGED_TO_BENCHMARK) {
      ...
      if (conditionsSize > 0) {
        ...
        for (let n = 0; n < conditionsSize; n++) {
          let conditionType = this._shiftInt();
          let condition = order_condition.Create(conditionType)
...
function handler_OPEN_ORDER(fields) {
  fields.shift();
...
function handler_COMPLETED_ORDER(fields) {
  fields.shift();
